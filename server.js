...
function send_game_update(socket, game_id, message) {
    ...
    io.of('/').to(game_id).allSockets().then((sockets) => {
        ...
        if (sockets.size >= 1) {
            ...
            if ((games[game_id].player_red.socket != first) &&
                (games[game_id].player_blue.socket != first)) {
                /** Player does not have a color */
                if (games[game_id].player_red.socket === "") {
                    /** This player should be red */
                    console.log("Red is assigned to: " + first);
                    games[game_id].player_red.socket = first;
                    games[game_id].player_red.username = players[first].username;
                }
        ...
        if (sockets.size >= 2) {
            ...
            if ((games[game_id].player_red.socket != second) &&
                (games[game_id].player_blue.socket != second)) {
                /** Player does not have a color */
                if (games[game_id].player_red.socket === "") {
                    /** This player should be red */
                    console.log("Red is assigned to: " + second);
                    games[game_id].player_red.socket = second;
                    games[game_id].player_red.username = players[second].username;
                }
                else if (games[game_id].player_blue.socket === "") {
                    /** This player should be blue */
